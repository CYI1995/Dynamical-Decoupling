...
BZ0 = np.kron(Z,I) + np.kron(I,Z) + np.kron(Z,Z)
BX0 = np.kron(X,I) + np.kron(I,X) + np.kron(X,X)

for i in range(N):

    b = 10/(1.5**(i+1))
    HSE = 2*b*np.kron(Z,BZ0) + b*np.kron(X,BX0) + b*np.kron(Y,BX0)
    ...
    for l in range(M):
        CDD = concatenated_single(CDD,pulse_x)

    for l in range(M):
        CDD = concatenated_single(CDD,pulse_z)
